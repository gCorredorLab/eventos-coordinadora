 ...
 * @file UploadDataEvents.ts
 * @description Caso de uso para realizar la carga masiva de eventos.
 * @verified NO
 * @status DEVELOP
 * @unitTests NO
 ...
 * @note
 *
 * Caso de uso para realizar la carga masiva de eventos desde un archivo Excel.
 */
...
/** @class UploadDataEvents */
export class UploadDataEvents {
  constructor(private bulkUploadService: IUploadDataService) {}

  /**
   * @method execute
   * @description Ejecuta la carga masiva de eventos desde un archivo Excel
  ...
  async execute(file: Readable): Promise<UploadDataResult> {
    ...
    return this.bulkUploadService.uploadEvents(data);
  }
}
