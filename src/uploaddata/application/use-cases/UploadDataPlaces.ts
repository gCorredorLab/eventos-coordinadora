 ...
 * @file UploadDataPlaces.ts
 * @description Caso de uso para realizar la carga masiva de lugares.
 * @verified NO
 * @status DEVELOP
 * @unitTests NO
 ...
 * @note
 *
 * Caso de uso para realizar la carga masiva de lugares desde un archivo Excel.
 */
...
/** @class UploadDataPlaces */
export class UploadDataPlaces {
  constructor(private bulkUploadService: IUploadDataService) {}

  /**
   * @method execute
   * @description Ejecuta la carga masiva de lugares desde un archivo Excel
  ...
  async execute(file: Readable): Promise<UploadDataResult> {
    ...
    return this.bulkUploadService.uploadPlaces(data);
  }
}
